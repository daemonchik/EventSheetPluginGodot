...
func get_value_at(index : int) -> Variant:
	if index < 0 or index >= _editors.size():
		push_error("ERROR: Invalid index %d for _editors array (size: %d)" % [index, _editors.size()])
		return null

	var editor = _editors[index]

	# Защита от неправильных типов
	if not (editor is Control):
		push_error("ERROR: Editor at index %d is not a Control: %s (type: %s)" % [index, str(editor), type_string(typeof(editor))])
		return null
	...
	push_warning("WARNING: Unknown editor type at index %d: %s" % [index, type_string(typeof(editor))])
	return null
...
func _add_property_editor(key : StringName, editor : Control, editor_signal : Signal, signal_handler : Callable):
	...
	# Отладка: проверяем, что editor действительно является Control
	if not (editor is Control):
		push_error("ERROR: Trying to add non-Control editor for key '%s': %s (type: %s)" % [key, str(editor), type_string(typeof(editor))])
		return
